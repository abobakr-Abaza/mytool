...
from app.core.tenancy import resolve_tenant_middleware
from app.core.tenancy.router import router as tenancy_router
...
# Register domain-resolution middleware (runs before auth so the
# public branding endpoint can resolve tenants without a token).
...
# Security headers: CSP, HSTS, nosniff, XFO
@app.middleware("http")
async def security_headers_middleware(request: Request, call_next):
    ...
    if settings.ENVIRONMENT == "production":
        ...
        response.headers["Content-Security-Policy"] = (
            ...
            "font-src 'self' data:; "
            "connect-src 'self' https://api.dentalpin.com; "
