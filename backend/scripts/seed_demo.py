...
async def seed_clinic(db: AsyncSession) -> Clinic:
    """Create the demo clinic + its cabinets + tenant branding."""
    from uuid import uuid4

    from app.core.tenancy.models import TenantBranding
    ...
    # Seed tenant branding with demo domain
    branding = TenantBranding(
        id=uuid4(),
        clinic_id=clinic.id,
        custom_domain="demo.dental.local",
        portal_title="Demo Dental Clinic",
        is_active=True,
    )
    db.add(branding)
    await db.flush()
...
async def main(lang: str = "en") -> None:
    ...
    print("\n" + "=" * 60)
    print(f"LaminarDent Demo Data Seeder ({lang_name})")
    print("=" * 60 + "\n")
